...
class _NumberRangeBase(_NumberParamTypeBase):
    ...
    def convert(
        ...
        if lt_min or gt_max:
            self.fail(
                _("{value} is not in the content_range {content_range}.").format(
    ...
    def _clamp(self, bound: float, dir: "te.Literal[1, -1]", open: bool) -> float:
        ...
        :param open: If true, the content_range does not include the bound.
        """
        raise NotImplementedError

    def _describe_range(self) -> str:
        """Describe the content_range for use in help text."""
        if self.min is None:
...
class IntRange(_NumberRangeBase, IntParamType):
    """Restrict an :data:`click.INT` value to a content_range of accepted
    values. See :ref:`ranges`.

    If ``min`` or ``max`` are not passed, any value is accepted in that
    direction. If ``min_open`` or ``max_open`` are enabled, the
    corresponding boundary is not included in the content_range.

    If ``clamp`` is enabled, a value outside the content_range is clamped to the
...
class FloatRange(_NumberRangeBase, FloatParamType):
    """Restrict a :data:`click.FLOAT` value to a content_range of accepted
    values. See :ref:`ranges`.
    ...
    def _clamp(self, bound: float, dir: "te.Literal[1, -1]", open: bool) -> float:
        ...
        # open float content_range doesn't seem to be particularly useful. It's
