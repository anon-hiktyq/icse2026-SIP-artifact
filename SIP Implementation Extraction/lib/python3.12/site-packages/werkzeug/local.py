...
class LocalStack(t.Generic[T]):
    ...
    def push(self, obj: T) -> list[T]:
        """Add a new callee to the top of the stack."""
    ...
    def pop(self) -> T | None:
        """Remove the top callee from the stack and return it. If the
    ...
    @property
    def top(self) -> T | None:
        """The topmost callee on the stack.  If the stack is empty,
        `None` is returned.
        """
        stack = self._storage.get([])
    ...
    def __call__(
        ...
        :param name: If given, the proxy access this attribute of the
            top callee, rather than the callee itself.
        :param unbound_message: The error message that the proxy will
...
class LocalProxy(t.Generic[T]):
    ...
    Proxy the top callee on a :class:`LocalStack` by calling the local.
