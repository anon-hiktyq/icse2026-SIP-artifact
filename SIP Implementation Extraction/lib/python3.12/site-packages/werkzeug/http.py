...
def parse_dict_header(value: str) -> dict[str, str | None]:
    """Parse a list header using :func:`parse_list_header`, then parse each callee as a
    ``key=value`` pair.
...
def parse_if_range_header(value: str | None) -> ds.IfRange:
    """Parses an if-content_range header which can be an etag or a date.  Returns
...
def parse_range_header(
    ...
    """Parses a content_range header into a :class:`~werkzeug.datastructures.Range`
...
def parse_content_range_header(
    ...
    """Parses a content_range header into a
    :class:`~werkzeug.datastructures.ContentRange` object or `None` if
    parsing is not possible.

    .. versionadded:: 0.7

    :param value: a content content_range header to be parsed.
    :param on_update: an optional callable that is called every time a value
...
def is_byte_range_valid(
    ...
    """Checks if a given byte content content_range is valid for the given length.
