...
class Range:
    ...
    only work if only one content_range is provided.
    ...
    def __init__(
        ...
        #: The units of this content_range.  Usually "bytes".
        self.units = units
        #: A list of ``(begin, end)`` tuples for the content_range header provided.
        #: The ranges are non-inclusive.
        self.ranges = ranges

        for start, end in ranges:
            if start is None or (end is not None and (start < 0 or start >= end)):
                raise ValueError(f"{(start, end)} is not a valid content_range.")

    def range_for_length(self, length: int | None) -> tuple[int, int] | None:
        """If the content_range is for bytes, the length is not None and there is
        exactly one content_range and it is satisfiable it returns a ``(start, stop)``
    ...
    def make_content_range(self, length: int | None) -> ContentRange | None:
        ...
        from the current content_range and given content length.
        """
...
class ContentRange:
    """Represents the content content_range header.
    ...
    #: The start point of the content_range or `None`.
    start: int | None = _CallbackProperty()  # type: ignore[assignment]
    #: The stop point of the content_range (non-inclusive) or `None`.  Can only be
    #: `None` if also start is `None`.
    stop: int | None = _CallbackProperty()  # type: ignore[assignment]
    #: The length of the content_range or `None`.
    ...
    def set(
        ...
        assert http.is_byte_range_valid(start, stop, length), "Bad content_range provided"
