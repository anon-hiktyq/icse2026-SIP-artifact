...
// Recursively resolve the type in the source code.
string parseType(DIType *Type) {
    if (auto *DerivedType = dyn_cast<DIDerivedType>(Type)) {
        string basicType = parseType(DerivedType->getBaseType());
        return basicType + "*";
    ...
    for (auto fun:FunTotal)
    {
        ...
        for (auto argVar:funArgVarMap[svfFunction])
        {
            ...
            if (it != varToTypeMap.end()) {
                ...
                llvm::outs() << "No type information found for: " << argVar->getName() << "\n";
            }
        }
        llvm::outs() << "\n";
    }
