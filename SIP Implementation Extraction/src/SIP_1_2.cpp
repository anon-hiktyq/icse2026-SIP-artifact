...
std::string parseType(DIType *Type) {
    ...
    if (auto *DerivedType = dyn_cast<DIDerivedType>(Type)) {
        ...
        // 其他派生类型处理...
        else if (DerivedType->getTag() == llvm::dwarf::DW_TAG_const_type) {
            return "const " + basicType;
        }
        else if (DerivedType->getTag() == llvm::dwarf::DW_TAG_volatile_type) {
            return "volatile " + basicType;
        }
        else if (DerivedType->getTag() == llvm::dwarf::DW_TAG_restrict_type) {
            return "restrict " + basicType;
        }
        else if (DerivedType->getTag() == llvm::dwarf::DW_TAG_reference_type) {
...
bool aliasWithParaAndItsPointsTO(Function *llvmFun, SVFValue *arg, SVFValue *storedValue, bool isGlobal){
    ...
    while(!worklist.empty()){
        ...
        if(targetObj->hasValue())
        {
            ...
            if (pta->alias(value, storedValue) == AliasResult::MayAlias) {
                if(isGlobal)
                {
                    funToOutputGlobalMap[svfFunction].insert(arg);
                }else
                {
        ...
        for(auto &inst : bb){
            if(auto storeInst = dyn_cast<StoreInst>(&inst)) {
                ...
                for (auto arg: args) {
                    ...
                    if(aliasWithParaAndItsPointsTO(llvmFun, svfArg, svfval, false) && strongUpdate)
                ...
                for (auto global: usedGlobals) {
                    ...
                    if(aliasWithParaAndItsPointsTO(llvmFun, svfGlobal, svfval, true) && strongUpdate)
                        funToStrongUpdateInst[llvmFun].insert(storeInst);
                }
...
//    }
    ...
    for(auto refGlobal : refGlobals){
        ...
        if(it == callerInputGloabls.end()){
            funToOutputGlobalMap[cs->getCaller()].insert(refGlobal);
            change = true;
        }
    }
