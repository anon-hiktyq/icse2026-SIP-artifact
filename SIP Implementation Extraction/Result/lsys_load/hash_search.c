...
void hash_searchFun(void *p) 
{
  ...
  do {
    ...
    {if (pIp->j <= l_castS2U(LUA_MAXINTEGER) / 2)
      ...
      {int hashkeyisempty_ret_1;IPCALL(hashkeyisempty,iphashkeyisempty_1,.t = pIp->t,.key = pIp->j,.ret = &hashkeyisempty_ret_1);if (hashkeyisempty_ret_1)  /* t[j] not present? */
  ...
  while ((pIp->j) - i > 1u) {  /* do a binary search between them */
    ...
    {int hashkeyisempty_ret_0;IPCALL(hashkeyisempty,iphashkeyisempty_0,.t = pIp->t,.key = m,.ret = &hashkeyisempty_ret_0);if (hashkeyisempty_ret_0) (pIp->j) = m;
    else i = m;}
  }
  *(pIp->ret) =  i;
}