...
void luaF_closeFun(void *p) 
{
  ...
  ptrdiff_t levelrel;{levelrel = savestack(pIp->L, pIp->level);}
  {IPCALL(luaF_closeupval,ipluaF_closeupval_0,.L = pIp->L,.level = pIp->level);}  /* first, close the upvalues */
  while ((pIp->L)->tbclist.p >= (pIp->level)) {  /* traverse tbc's down to that level */
    StkId tbc = (pIp->L)->tbclist.p;  /* get variable index */
    {IPCALL(poptbclist,ippoptbclist_0,.L = pIp->L);}  /* remove it from list */
    {IPCALL(prepcallclosemth,ipprepcallclosemth_0,.L = pIp->L,.level = tbc,.status = pIp->status,.yy = pIp->yy);}  /* close variable */
