...
void precallCFun(void *p) 
{
  ...
  {CallInfo * prepCallInfo_ret_0;IPCALL(prepCallInfo,ipprepCallInfo_0,.L = pIp->L,.func = pIp->func,.status = status | CIST_C,.top = L->top.p + LUA_MINSTACK,.ret = prepCallInfo_ret_0);pIp->L->ci = ci = prepCallInfo_ret_0
;}
  {lua_assert(ci->top.p <= pIp->L->stack_last.p);}
  {if (l_unlikely(pIp->L->hookmask & LUA_MASKCALL)) {
    int narg;{narg = cast_int(pIp->L->top.p - pIp->func) - 1;}
