  ...
  while ((e = strchr((pIp->fmt), '%')) != NULL) {
    ...
    switch (*(e + 1)) {  /* conversion specifier */
      case 's': {  /* zero-terminated string */
        ...
        if (s == NULL) s = "(null)";
        {IPCALL(addstr2buff,ipaddstr2buff_3,.buff = &buff,.str = s,.slen = strlen(s));}
        break;
      }
      case 'c': {  /* an 'int' as a character */
        char c;{c = cast_char(va_arg(pIp->argp, int));}
        {IPCALL(addstr2buff,ipaddstr2buff_2,.buff = &buff,.str = &c,.slen = sizeof(char));}
        break;
      }
      case 'd': {  /* an 'int' */
        TValue num;
      ...
      default: {
        {IPCALL(addstr2buff,ipaddstr2buff_0,.buff = &buff,.str = e,.slen = 2);}  /* keep unknown format in the result */
        break;
      }
    }
    (pIp->fmt) = e + 2;  /* skip '%' and the specifier */
  }
  {IPCALL(addstr2buff,ipaddstr2buff_1,.buff = &buff,.str = pIp->pIp->fmt,.slen = strlen(pIp->fmt));}  /* rest of 'fmt' */
