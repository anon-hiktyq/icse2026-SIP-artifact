  ...
  for (;;) {
    ...
    vmdispatch (GET_OPCODE(i)) {
      ...
      vmcase(OP_GETTABUP) {
        ...
        {if (tagisempty(tag))
          {lu_byte luaV_finishget_ret_4;IPCALL(luaV_finishget,ipluaV_finishget_4,.L = pIp->L,.t = upval,.key = rc,.val = ra,.tag = tag,.ret = &luaV_finishget_ret_4);Protect(luaV_finishget(pIp->L, upval, rc, ra, tag));}}
        vmbreak;
      }
      vmcase(OP_GETTABLE) {
        StkId ra;{ra = RA(i);}
      ...
      vmcase(OP_SELF) {
        ...
        {luaV_fastget(rb, key, s2v(ra), luaH_getshortstr, tag);}
        {if (tagisempty(tag))
          {lu_byte luaV_finishget_ret_0;IPCALL(luaV_finishget,ipluaV_finishget_0,.L = pIp->L,.t = rb,.key = rc,.val = ra,.tag = tag,.ret = &luaV_finishget_ret_0);Protect(luaV_finishget(pIp->L, rb, rc, ra, tag));}}
        vmbreak;
      }
      vmcase(OP_ADDI) {
      ...
      vmcase(OP_UNM) {
        ...
        {if (ttisinteger(rb)) {
          ...
          {IPCALL(luaT_trybinTM,ipluaT_trybinTM_1,.L = pIp->L,.p1 = rb,.p2 = rb,.res = ra,.event = TM_UNM);Protect(luaT_trybinTM(pIp->L, rb, rb, ra, TM_UNM));}}}
        vmbreak;
      }
      vmcase(OP_BNOT) {
        StkId ra;{ra = RA(i);}
        ...
        {if (tointegerns(rb, &ib)) {
          ...
          {IPCALL(luaT_trybinTM,ipluaT_trybinTM_0,.L = pIp->L,.p1 = rb,.p2 = rb,.res = ra,.event = TM_BNOT);Protect(luaT_trybinTM(pIp->L, rb, rb, ra, TM_BNOT));}}
        vmbreak;
      }
      vmcase(OP_NOT) {
        StkId ra;{ra = RA(i);}
      ...
      vmcase(OP_TBC) {
        ...
        {IPCALL(luaF_newtbcupval,ipluaF_newtbcupval_1,.L = pIp->L,.level = ra);halfProtect(luaF_newtbcupval(pIp->L, ra));}
        vmbreak;
      }
      vmcase(OP_JMP) {
        {dojump(pIp->ci, i, 0);}
       ...
       */
       ...
       {setobj2s(pIp->L, ra + 2, &temp);}
        ...
        {IPCALL(luaF_newtbcupval,ipluaF_newtbcupval_0,.L = pIp->L,.level = ra + 2);halfProtect(luaF_newtbcupval(pIp->L, ra + 2));}
