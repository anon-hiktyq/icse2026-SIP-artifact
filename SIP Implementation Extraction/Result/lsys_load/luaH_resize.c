...
void luaH_resizeFun(void *p) 
{
  ...
  if ((pIp->newasize) < oldasize) {  /* will array shrink? */
    ...
    {IPCALL(exchangehashpart,ipexchangehashpart_0,.t1 = pIp->t,.t2 = &newt);}  /* pretend table has new hash */
    {IPCALL(reinsertOldSlice,ipreinsertOldSlice_0,.t = pIp->t,.oldasize = oldasize,.newasize = pIp->newasize);}
    {IPCALL(exchangehashpart,ipexchangehashpart_1,.t1 = pIp->t,.t2 = &newt);}  /* restore old hash (in case of errors) */
  }
  /* allocate new array */
