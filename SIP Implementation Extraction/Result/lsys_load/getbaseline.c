...
void getbaselineFun(void *p) 
{
  ...
  if ((pIp->f)->sizeabslineinfo == 0 || (pIp->pc) < (pIp->f)->abslineinfo[0].pc) {
    *(pIp->basepc) = -1;  /* start from the beginning */
    *(pIp->ret) =  (pIp->f)->linedefined;
  }
  else {
    ...
    while (i + 1 < (pIp->f)->sizeabslineinfo && (pIp->pc) >= (pIp->f)->abslineinfo[i + 1].pc)
      i++;  /* low estimate; adjust it */
    *(pIp->basepc) = (pIp->f)->abslineinfo[i].pc;
    *(pIp->ret) =  (pIp->f)->abslineinfo[i].line;
  }
}