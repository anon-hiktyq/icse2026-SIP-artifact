...
void sweepgenFun(void *p) 
{
  ...
  l_mem addedold = 0;
  int white;{white = luaC_white(pIp->g);}
  GCObject *curr;
      ...
      *(pIp->p) = curr->next;  /* remove 'curr' from list */
      ...
      if (age == G_NEW) {  /* new objects go back to white */
        ...
        {if (getage(curr) == G_OLD1) {
          {l_mem objsize_ret_0;IPCALL(objsize,ipobjsize_0,.o = curr,.ret = &objsize_ret_0);addedold += objsize_ret_0;}  /* bytes becoming old */
