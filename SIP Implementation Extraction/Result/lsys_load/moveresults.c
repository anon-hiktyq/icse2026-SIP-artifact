...
void moveresultsFun(void *p) 
{
  ...
  switch ((pIp->fwanted)) {  /* handle typical cases separately */
    ...
    case 1 + 1:  /* one value needed */
      ...
      return;
    case LUA_MULTRET + 1:
      {IPCALL(genmoveresults,ipgenmoveresults_0,.L = pIp->L,.res = pIp->res,.nres = pIp->nres,.wanted = pIp->nres);}  /* we want all results */
      break;
    default: {  /* two/more results and/or to-be-closed variables */
      ...
      {IPCALL(genmoveresults,ipgenmoveresults_1,.L = pIp->L,.res = pIp->res,.nres = pIp->nres,.wanted = wanted);}
      break;
    }
  }
}