...
void lua_rotateFun(void *p) 
{
  ...
  {api_check(pIp->L, (pIp->n >= 0 ? pIp->n : -pIp->n) <= (t - p + 1), "invalid 'pIp->n'");}
  m = ((pIp->n) >= 0 ? t - (pIp->n) : p - (pIp->n) - 1);  /* end of prefix */
  {IPCALL(reverse,ipreverse_0,.L = pIp->L,.from = p,.to = m);}  /* reverse the prefix with length 'n' */
  {IPCALL(reverse,ipreverse_1,.L = pIp->L,.from = m + 1,.to = t);}  /* reverse the suffix */
  {IPCALL(reverse,ipreverse_2,.L = pIp->L,.from = p,.to = t);}  /* reverse the entire segment */
  {lua_unlock(pIp->L);}
}