...
void luaO_tostringFun(void *p) 
{
  ...
  {TString * luaS_newlstr_ret_0;IPCALL(luaS_newlstr,ipluaS_newlstr_0,.L = pIp->L,.str = buff,.l = len,.ret = luaS_newlstr_ret_0);setsvalue(pIp->L, pIp->obj, luaS_newlstr(pIp->L, buff, len));}
}