...
void lua_getinfoFun(void *p) 
{
  ...
  {int auxgetinfo_ret_0;IPCALL(auxgetinfo,ipauxgetinfo_0,.L = pIp->L,.what = pIp->what,.ar = pIp->ar,.f = cl,.ci = ci,.ret = &auxgetinfo_ret_0);status = auxgetinfo_ret_0;}
  if (strchr((pIp->what), 'f')) {
    {setobj2s(pIp->L, pIp->L->top.p, func);}
    {api_incr_top(pIp->L);}
  }
  if (strchr((pIp->what), 'L'))
