...
void l_str2intFun(void *p) 
{
  ...
  {int isneg_ret_1;IPCALL(isneg,ipisneg_0,.s = pIp->&s,.ret = pIp->&isneg_ret_1);neg = isneg_ret_1;}
  if (pIp -> s[0] == '0' &&
      (pIp -> s[1] == 'x' || pIp -> s[1] == 'X')) {  /* hex? */
    pIp -> s += 2;  /* skip '0x' */
    for (; lisxdigit(cast_uchar(pIp -> *s)); pIp -> s++) {
      {lu_byte luaO_hexavalue_ret_0;IPCALL(luaO_hexavalue,ipluaO_hexavalue_0,.c = *s,.ret = pIp->&luaO_hexavalue_ret_0);a = a * 16 + luaO_hexavalue_ret_0;}
      empty = 0;
    }
  }
  else {  /* decimal */
  ...
  if (empty || pIp -> *s != '\0') return NULL;  /* something wrong in the numeral */
  else {
