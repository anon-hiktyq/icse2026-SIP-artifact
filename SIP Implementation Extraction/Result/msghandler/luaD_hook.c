...
void luaD_hookFun(void *p) 
{
  ...
  if (hook && (pIp->L)->allowhook) {  /* make sure there is a hook */
    ...
    ptrdiff_t top;{top = savestack(pIp->L, pIp->L->top.p);}  /* preserve original 'top' */
    ptrdiff_t ci_top;{ci_top = savestack(pIp->L, ci->top.p);}  /* idem for 'ci->top' */
    lua_Debug ar;
    ar.event = (pIp->event);
    ar.currentline = (pIp->line);
    ar.i_ci = ci;
    (pIp->L)->transferinfo.ftransfer = (pIp->ftransfer);
    (pIp->L)->transferinfo.ntransfer = (pIp->ntransfer);
