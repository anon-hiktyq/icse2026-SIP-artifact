...
void lua_setmetatableFun(void *p) 
{
  ...
  {TValue * index2value_ret_0;IPCALL(index2value,ipindex2value_0,.L = pIp->L,.idx = objindex,.ret = pIp->index2value_ret_0);obj = index2value_ret_0;}
  ...
  switch (ttype(obj)) {
    case LUA_TTABLE: {
      ...
      if (mt) {
        ...
        {IPCALL(luaC_checkfinalizer,ipluaC_checkfinalizer_1);}
      }
      break;
    }
    case LUA_TUSERDATA: {
      ...
      if (mt) {
        ...
        {IPCALL(luaC_checkfinalizer,ipluaC_checkfinalizer_0);}
      }
      break;
    }
    default: {
