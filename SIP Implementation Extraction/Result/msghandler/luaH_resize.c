...
void luaH_resizeFun(void *p) 
{
  ...
  newt.flags = 0;
  {IPCALL(setnodevector,ipsetnodevector_0,.L = pIp->L,.t = pIp->&newt,.size = nhsize);}
  if (pIp -> newasize < oldasize) {  /* will array shrink? */
    /* re-insert into the new hash the elements from vanishing slice */
    {IPCALL(exchangehashpart,ipexchangehashpart_2,.t1 = t,.t2 = &newt);}  /* pretend table has new hash */
    {IPCALL(reinsertOldSlice,ipreinsertOldSlice_0,.t = pIp->t,.oldasize = oldasize,.newasize = pIp->newasize);}
    {IPCALL(exchangehashpart,ipexchangehashpart_1,.t1 = t,.t2 = &newt);}  /* restore old hash (in case of errors) */
  }
  /* allocate new array */
  ...
  if (l_unlikely(newarray == NULL && pIp -> newasize > 0)) {  /* allocation failed? */
    {IPCALL(freehash,ipfreehash_1,.L = pIp->L,.t = pIp->&newt);}  /* release new hash part */
    luaM_error(pIp -> L);  /* raise error (with array unchanged) */
  }
  /* allocation ok; initialize new part of the array */
  {IPCALL(exchangehashpart,ipexchangehashpart_0,.t1 = t,.t2 = &newt);}  /* 'pIp -> t' has the new hash ('newt' has the old) */
  ...
  {IPCALL(freehash,ipfreehash_0,.L = pIp->L,.t = pIp->&newt);}  /* free old hash part */
}