...
void luaS_resizeFun(void *p) 
{
  ...
  if ((pIp->nsize) < osize)  /* shrinking table? */
    {IPCALL(tablerehash,iptablerehash_0,.vect = tb->hash,.osize = osize,.nsize = pIp->nsize);}  /* depopulate shrinking part */
  {newvect = luaM_reallocvector(pIp->L, tb->hash, osize, pIp->nsize, TString*);}
  {if (l_unlikely(newvect == NULL)) {  /* reallocation failed? */
    if ((pIp->nsize) < osize)  /* was it shrinking table? */
      {IPCALL(tablerehash,iptablerehash_1,.vect = tb->hash,.osize = pIp->nsize,.nsize = osize);}  /* restore to original size */
