...
void luaT_callorderTMFun(void *p) 
{
  ...
  int tag;{int callbinTM_ret_0; IPCALL(callbinTM,ipcallbinTM_0,.L = pIp->L,.p1 = pIp->p1,.p2 = pIp->p2,.res = L->top.p,.event = pIp->event,.ret = &callbinTM_ret_0);tag = callbinTM_ret_0;}  /* try original event */
