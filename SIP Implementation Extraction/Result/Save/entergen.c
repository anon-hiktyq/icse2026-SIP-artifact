...
void entergenFun(void *p) 
{
  ...
  {IPCALL(luaC_runtilstate,ipluaC_runtilstate_0,.L = pIp->L,.state = GCSpause,.fast = 1);}  /* prepare to start a new cycle */
  {IPCALL(luaC_runtilstate,ipluaC_runtilstate_1,.L = pIp->L,.state = GCSpropagate,.fast = 1);}  /* start new cycle */
