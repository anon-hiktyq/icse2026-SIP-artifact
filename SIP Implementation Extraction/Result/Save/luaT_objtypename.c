...
void luaT_objtypenameFun(void *p) 
{
  ...
  {if ((ttistable(pIp->o) && (mt = hvalue(pIp->o)->metatable) != NULL) ||
    ...
    const TValue name;{const TValue * luaH_Hgetshortstr_ret_0; IPCALL(luaH_Hgetshortstr,ipluaH_Hgetshortstr_0,.ret = luaH_Hgetshortstr_ret_0);name = luaH_Hgetshortstr_ret_0;}
