...
void reallymarkobjectFun(void *p) 
{
  ...
  {l_mem objsize_ret_0;IPCALL(objsize,ipobjsize_0,.o = pIp->o,.ret = &objsize_ret_0);pIp->g->GCmarked += objsize_ret_0;}
  switch ((pIp->o)->tt) {
    case LUA_VSHRSTR:
    case LUA_VLNGSTR: {
    ...
    case LUA_VUSERDATA: {
      ...
      if (u->nuvalue == 0) {  /* no user values? */
        {markobjectN(pIp->g, u->metatable);}  /* mark its metatable */
