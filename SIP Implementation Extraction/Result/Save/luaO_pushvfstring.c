  ...
  while ((e = strchr((pIp->fmt), '%')) != NULL) {
    ...
    switch (*(e + 1)) {  /* conversion specifier */
      case 's': {  /* zero-terminated string */
        ...
        if (s == NULL) s = "(null)";
        {IPCALL(addstr2buff,ipaddstr2buff_0,.buff = &buff,.str = s,.slen = strlen(s));}
        break;
      }
      ...
      case '%': {
        {IPCALL(addstr2buff,ipaddstr2buff_1,.buff = &buff,.str = "%",.slen = 1);}
        break;
      }
      default: {
        {IPCALL(addstr2buff,ipaddstr2buff_4,.buff = &buff,.str = e,.slen = 2);}  /* keep unknown format in the result */
        break;
      }
    }
