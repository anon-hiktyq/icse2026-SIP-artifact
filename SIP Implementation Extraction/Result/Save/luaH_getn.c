...
void luaH_getnFun(void *p) 
{
  ...
  if (asize > 0) {  /* is there an array part? */
    ...
    {int arraykeyisempty_ret_1;IPCALL(arraykeyisempty,iparraykeyisempty_1,.t = pIp->t,.key = limit,.ret = &arraykeyisempty_ret_1);if (arraykeyisempty_ret_1) {  /* t[limit] empty? */
      /* there must be a border before 'limit' */
      unsigned i;
      /* look for a border in the vicinity of the hint */
      for (i = 0; i < maxvicinity && limit > 1; i++) {
        limit--;
        {int arraykeyisempty_ret_0;IPCALL(arraykeyisempty,iparraykeyisempty_0,.t = pIp->t,.key = limit,.ret = &arraykeyisempty_ret_0);if (!arraykeyisempty_ret_0)
