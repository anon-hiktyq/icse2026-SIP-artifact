  ...
  for (;;) {
    ...
    vmdispatch (GET_OPCODE(i)) {
      ...
      vmcase(OP_GETTABUP) {
        ...
        {if (tagisempty(tag))
          {lu_byte luaV_finishget_ret_2;IPCALL(luaV_finishget,ipluaV_finishget_2,.L = pIp->L,.t = upval,.key = rc,.val = ra,.tag = tag,.ret = &luaV_finishget_ret_2);Protect(luaV_finishget(pIp->L, upval, rc, ra, tag));}}
        vmbreak;
      }
      vmcase(OP_GETTABLE) {
        StkId ra;{ra = RA(i);}
      ...
      vmcase(OP_TAILCALL) {
        ...
        {int luaD_pretailcall_ret_0;IPCALL(luaD_pretailcall,ipluaD_pretailcall_0,.ret = &luaD_pretailcall_ret_0);if ((n = luaD_pretailcall_ret_0) < 0)  /* Lua function? */
          ...
          {IPCALL(luaD_poscall,ipluaD_poscall_0);}  /* finish caller */
      ...
      vmcase(OP_RETURN) {
        ...
        {IPCALL(luaD_poscall,ipluaD_poscall_1);}
        {updatetrap(pIp->ci);}  /* 'luaD_poscall' can change hooks */
        goto ret;
      }
      vmcase(OP_RETURN0) {
      ...
      vmcase(OP_RETURN1) {
        {if (l_unlikely(pIp->L->hookmask)) {
          ...
          (pIp->L)->top.p = ra + 1;
          {savepc(pIp->ci);}
          {IPCALL(luaD_poscall,ipluaD_poscall_2);}  /* no hurry... */
          trap = 1;
        }
        else {  /* do the 'poscall' here */
          int nres;{nres = get_nresults(pIp->ci->callstatus);}
