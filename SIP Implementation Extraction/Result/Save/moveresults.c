...
void moveresultsFun(void *p) 
{
  ...
  switch (pIp -> fwanted) {  /* handle typical cases separately */
    ...
    case 1 + 1:  /* one value needed */
      ...
      return;
    case LUA_MULTRET + 1:
      {IPCALL(genmoveresults,ipgenmoveresults_1,.L = pIp->L,.res = pIp->res,.nres = pIp->nres,.wanted = nres);}  /* we want all results */
      break;
    default: {  /* two/more results and/or to-be-closed variables */
      int wanted = get_nresults(pIp -> fwanted);
      if (pIp -> fwanted & CIST_TBC) {  /* to-be-closed variables? */
        pIp -> L->ci->u2.pIp -> nres = pIp -> nres;
        ...
        if (pIp -> L->hookmask) {  /* if needed, call hook after '__close's */
          ptrdiff_t savedres = savestack(pIp -> L, pIp -> res);
