...
void findsetregFun(void *p) 
{
  ...
  int pc;
  int setreg = -1;  /* keep last instruction that changed 'pIp -> reg' */
  ...
  for (pc = 0; pc < pIp -> lastpc; pc++) {
    ...
    if (change)
      setreg = {int filterpc_ret_0;IPCALL(filterpc,ipfilterpc_0,.pc = pc,.jmptarget = jmptarget,.ret = pIp->&filterpc_ret_0);}
  }
  *(pIp -> ret) = setreg;
}