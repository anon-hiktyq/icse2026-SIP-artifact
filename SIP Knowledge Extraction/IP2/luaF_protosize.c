...
void luaF_protosizeFun(void *p) 
{
  ...
  lu_mem pIp -> sz = cast(lu_mem, sizeof(Proto))
  ...
  if (!(pIp -> p->flag & PF_FIXED)) {
    pIp -> sz += cast_uint(pIp -> p->sizecode) * sizeof(Instruction);
    pIp -> sz += cast_uint(pIp -> p->sizelineinfo) * sizeof(lu_byte);
    pIp -> sz += cast_uint(pIp -> p->sizeabslineinfo) * sizeof(AbsLineInfo);
  }
  *(pIp -> ret) = sz;
}