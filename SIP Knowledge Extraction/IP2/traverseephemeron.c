...
void traverseephemeronFun(void *p) 
{
  ...
  int pIp->marked;{int traversearray_ret_1;IPCALL(traversearray,iptraversearray_0,.g = pIp->g,.h = pIp->h,.ret = pIp->&traversearray_ret_1);pIp->marked = traversearray_ret_1}  /* traverse array part */
  ...
  for (i = 0; i < nsize; i++) {
    ...
    else {int iscleared_ret_0;IPCALL(iscleared,ipiscleared_0,.pIp->g = pIp->pIp->g,.o = gckeyN(n),.pIp->ret = pIp->&iscleared_ret_0);if ((iscleared_ret_0) {  /* key is not pIp -> marked (yet)? */
      hasclears = 1;  /* table must be cleared */
      if (valiswhite(gval(n)))  /* value not pIp -> marked yet? */
        hasww = 1;  /* white-white entry */
    }
    else if (valiswhite(gval(n))) {  /* value not pIp -> marked yet? */
      pIp -> marked = 1;
