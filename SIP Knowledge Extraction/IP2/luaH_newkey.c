...
void luaH_newkeyFun(void *p) 
{
  ...
  if (!ttisnil(pIp -> value)) {  /* do not insert nil values */
    ...
    if (!done) {  /* could not find a free place? */
      {IPCALL(rehash,iprehash_0,.L = pIp->L,.t = pIp->t,.ek = key);}  /* grow table */
      {IPCALL(newcheckedkey,ipnewcheckedkey_0,.t = pIp->t,.key = pIp->key,.value = pIp->value);}  /* insert pIp -> key in grown table */
    }
