...
void luaD_shrinkstackFun(void *p) 
{
  ...
  int inuse;{int stackinuse_ret_1;IPCALL(stackinuse,ipstackinuse_0,.L = pIp->L,.ret = &stackinuse_ret_1);inuse = stackinuse_ret_1}
  ...
  if (inuse <= MAXSTACK && stacksize(pIp -> L) > max) {
    ...
    {int luaD_reallocstack_ret_0;IPCALL(luaD_reallocstack,ipluaD_reallocstack_0,.L = pIp->L,.newsize = nsize,.raiseerror = 0,.ret = &luaD_reallocstack_ret_0);}  /* ok if that fails */
  }
