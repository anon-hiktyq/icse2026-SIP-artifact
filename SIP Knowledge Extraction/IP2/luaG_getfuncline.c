...
void luaG_getfunclineFun(void *p) 
{
  ...
  if (pIp -> f->lineinfo == NULL)  /* no debug information? */
    *(pIp -> ret) = -1;
    ...
    int basepc;
    int baseline;{int getbaseline_ret_0;IPCALL(getbaseline,ipgetbaseline_0,.f = pIp->f,.pc = pIp->pc,.basepc = &basepc,.ret = pIp->&getbaseline_ret_0);baseline = getbaseline_ret_0}
